...
*/
function Inspector(divid, selectionchangecallback, diagram, options) {

    var mainDiv = document.getElementById(divid);
    //mainDiv.className = "theme-panel";
    mainDiv.innerHTML = "";
    this._div = mainDiv;
    this._selectionCallback = selectionchangecallback;
...
Inspector.prototype.inspectObject = function (object) {
    ...
    this._selectionCallback(inspectedObject);
    this.inspectedObject = inspectedObject;
    ...
    //// setting initial link state

    var linkstate = false;
    if (data['fromPort'] != undefined) {
        if ((data['from'] == data['to']) && (data['ExecutionTime'] == undefined)) {

            data['ExecutionTime'] = 0;
        }
        if ((data['text'] == undefined) && ((data['from'] != data['to']))) {
            data['text'] = 1;
    ...
    for (var k in declaredProperties) {
        ...
        if ((linkstate == true) && (data['fromPort'] != undefined)) {
            if ((k != 'text') || (k != 'ExecutionOrder') || (k != 'ExecutionTime')) {
        ...
        var isLink = data['ExecutionOrder'] != undefined;
        var sData = { store: declaredProperties[k].store };
        var fromNodeType = 0;

        if (isLink && k == "text") {
            var storelist = $('#drpLinkType option').map(function () {
                if (this.value != "") {
                    return this.value;
                }
            }).get();

            var storetextlist = $('#drpLinkType option').map(function () {
                if (this.text != "Select your option") {
                    return this.text;
                }
            }).get()

            sData['store'] = storelist;
            sData['storetextList'] = storetextlist;

            //// for getting from node type
            var nodeArray = this._diagram.model.nodeDataArray;
            var fromNode = getNode(nodeArray, data.from);
            fromNodeType = fromNode.type;
            //// end getting from node type
        }

        if (data.type != '8') {
            if (defaultValue != '') {
                this.buildPropertyRow(k, defaultValue || "", sData.store, sData.storetextList, fromNodeType)
            }
        }
        else {
            this.buildPropertyRow(k, defaultValue || "", sData.store, sData.storetextList, fromNodeType)
        ...
        //mainDiv.appendChild(this.buildPropertyRow(k, defaultValue || "", sData.store));
    }
    ...
    if (this.includesOwnProperties) {
        for (var k in data) {
            ...
            this.buildPropertyRow(k, data[k]);

            //parentDiv.appendChild(this.buildPropertyRow(k, data[k]));
        }
    }
...
*/
Inspector.prototype.buildPropertyRow = function (propertyName, propertyValue, store, storeTextList, fromNodeType) {
    var mainDiv = this._div;
    var decProp = this.declaredProperties[propertyName];
    var childDiv = document.createElement("div");
    childDiv.setAttribute('class', 'theme-option');

    var span = document.createElement('label');

    if (decProp.altname) {
        span.innerHTML = decProp.altname;
    }
    else {
    ...
    if (store != undefined) {
        ...
        for (var i = 0; i < store.length; i++) {
            ...
            option.text = storeTextList[i]
            input.add(option);
        }
        if ((fromNodeType == "8") || (fromNodeType == "7") || (fromNodeType == "6") || (fromNodeType == "2") || (fromNodeType == "3") || (fromNodeType == "10") || (fromNodeType == "5")) {
            input.value = "2";
        ...
        input.setAttribute('class', 'layout-style-option form-control ');
        input.setAttribute('style', 'border-radius:4px!important;margin-bottom: 20px;');
        input.setAttribute('id', 'linkTypes');
        input.setAttribute('data-code', 'linkTypes');
        ...
        if (decProp !== undefined) {
            var type = 'text';
            if (typeof decProp['type'] !== 'undefined')
                type = decProp['type'];
            ...
            if (decProp.pattern)
                input.setAttribute('pattern', decProp.pattern);
            if (decProp.min)
            {
                input.setAttribute('min', decProp.min);
            }
            if (decProp.max) {
                input.setAttribute('max', decProp.max);
            }
        }
...
Inspector.prototype.updateAllHTML = function () {
    ...
    this._selectionCallback(this.inspectedObject);
...
Inspector.prototype.updateAllProperties = function () {
    ...
    var isLink = data['ExecutionOrder'];
    ...
    for (var name in inspectedProps) {
        ...
        switch (type) {
            case "boolean":
                ...
                break;
            case "number":
                if (typeof decProp['isInteger'] === 'undefined')
                    value = parseFloat(value);
                else
                    value = decProp.isInteger ? parseInt(value) : parseFloat(value);
                break;
        }
