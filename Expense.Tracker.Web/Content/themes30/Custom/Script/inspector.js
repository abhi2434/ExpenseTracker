...
Inspector.prototype.inspectObject = function (object) {
    ...
    parentDiv.setAttribute('class', 'theme-options');
    parentDiv.setAttribute('style', 'top:0px;width:230px;');
    ...
    for (var k in declaredProperties) {
        ...
        var isLink = data['points'] != undefined;
        var sData = { store: declaredProperties[k].store };
        if (isLink)
            sData['store'] = declaredProperties[k].linkStore;

        parentDiv.appendChild(this.buildPropertyRow(k, defaultValue || "", sData.store));
    }
...
*/
Inspector.prototype.buildPropertyRow = function (propertyName, propertyValue, store) {
    ...
    if (store != undefined) {
        ...
        for (var i = 0; i < store.length; i++) {
            ...
            option.text = option.value = store[i];
            input.add(option);
        }
    }
    else {
    ...
    input.setAttribute('class', 'layout-style-option form-control input-sm');
    ...
    for (var name in inspectedProps) {
        ...
        switch (type) {
            case "boolean":
                ...
                break;
            case "number":
                if (typeof decPro['isInteger'] === 'undefined')
                    value = parseFloat(value);
                else
                    value = decPro.isInteger ? parseInt(value) : parseFloat(value);
                break;
        }
